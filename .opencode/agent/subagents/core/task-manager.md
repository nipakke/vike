      ...
      <process>
        1. Create directory:
          ...
          3. Create subtask_NN.json for each task:
              ...
              **FRONTEND RULE**: If a task involves UI design or frontend implementation:
              1. Set `suggested_agent`: "frontend-specialist"
              2. Include `.opencode/context/core/standards/code-quality.md` in `context_files`.
              3. Ensure `acceptance_criteria` includes "Responsive at all breakpoints".
              4. **PARALLELIZATION**: Frontend tasks can run in parallel (`parallel: true`) since design work is isolated and doesn't affect backend/logic implementation. Only mark `parallel: false` if frontend depends on backend API contracts or data structures.
